...
def plot_clim(flag_data,
    ...
    if seasons is None:
        ...
        ds_season = flag_data.sel({kwargs["time_name"]: flag_data[kwargs["time_name"]].dt.month.isin(seasons)})
...
def plot_step(flag_data,
    ...
    ds[variable].plot.contour(ax=ax, levels=np.asarray(contour_level).tolist(), transform=data_crs, linewidths=2, colors="black")
    ds["flag"].where(ds["flag"] > 0).plot.contourf(ax=ax, colors=["white", color_events], levels=[0, 0.5], transform=data_crs, add_colorbar=False)
...
def plot_tracks(data,
    ...
    data.isel({kwargs["time_name"]: 0}).plot.contourf(ax=ax, transform=data_crs, add_colorbar=False, alpha=0)
