...
def properties_per_timestep(date, lodf, data, intensity, periodic_add, *args, **kwargs):
    ...
    ds["weight"] = xr.DataArray(np.ones((kwargs["nlat"], kwargs["nlon"])) * np.array((111 * 1 * 111 * 1 * weight_lat)).astype(np.float32)[:, None], dims=[kwargs["lat_name"], kwargs["lon_name"]])
    ...
    def properties_per_event(df):
        ...
        temp = ds.isel({kwargs["lat_name"]: df.y.to_xarray(), kwargs["lon_name"]: df.x.to_xarray()})
        area = np.round(np.sum(temp.weight.values), 2)
        mean_var = np.round(np.average(temp.vars.values), 2)
        com = np.round(np.sum(df.multiply((temp.weight * temp.vars).values, axis=0), axis=0) / sum((temp.weight * temp.vars).values), 2)

        # map coordinates of the event to the original grid
        x_original = data[kwargs["lon_name"]].values[df.x.values.astype("int") % kwargs["nlon"]]
        y_original = data[kwargs["lat_name"]].values[df.y.values.astype("int") % kwargs["nlat"]]
        geo_mp = MultiPoint(np.c_[x_original, y_original])

        com_x = data[kwargs["lon_name"]].values[(np.round(com.x) % kwargs["nlon"]).astype("int")]
        com_y = data[kwargs["lat_name"]].values[(np.round(com.y) % kwargs["nlat"]).astype("int")]
        ...
        if "intensity" in ds.variables:
            prop_dict["intensity"] = np.round(np.sum((temp.weight * temp.intensity).values) / area, 2)
    ...
    return gpd.GeoDataFrame(pd.DataFrame([item[1] for item in properties]),
