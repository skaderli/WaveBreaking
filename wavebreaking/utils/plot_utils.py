...
def get_new_cmap(color_palette):
    ...
    return colors.LinearSegmentedColormap.from_list('trunc({n},{a:.2f},{b:.2f})'.format(n=cmap.name, a=0.3, b=1),cmap(np.linspace(0.3, 1, 100)))
...
def add_grid_lines(axes):
    ...
    gr = axes.gridlines(draw_labels=True, color="black", linestyle="dotted", linewidth = 1.1)
