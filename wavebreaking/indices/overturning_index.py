...
def calculate_overturnings(data, contour_level, range_group = 500, min_exp = 5, intensity = None, periodic_add = 120, *args, **kwargs):
    ...
    The overturning region is represented by a rectangle enclosing all overturning contour points.
    ...
    -------
        overturnings: geopandas.GeoDataFrame
            GeoDataFrame containing different characteristics of the overturning events
                ...
                "mean_var": mean of the variable used for the overturning calculations over all overturning grid cells
                ...
                "geometry": MultiPoint object with the coordinate points of the overturning in the format (x,y)
    """
    
    #filter contours
    contours = kwargs["contours"]
    contours = contours[contours.exp_lon == contours.exp_lon.max()].reset_index(drop=True)

    #loop over contours
    overturnings = []
    for (index,row) in tqdm(contours.iterrows(), total = contours.shape[0], leave = True, position = 0):
        ...
        def check_duplicates(df):
            ...
            temp = pd.concat([df.y1, df.x1%kwargs["nlat"], df.y2, df.x2%kwargs["nlon"]], axis = 1)
        ...
        if df_ot.empty:
            ...
            overturning_grids = [pd.DataFrame(ot_to_grid(row), columns = ["y", "x"]) for index,row in df_ot.iterrows()]
            overturnings.append(properties_per_timestep(row.date, overturning_grids, data, intensity, periodic_add, *args, **kwargs))
