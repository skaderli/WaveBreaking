...
def calculate_streamers(data, contour_level, geo_dis = 800, cont_dis = 1500, intensity = None, periodic_add = 120, *args, **kwargs):
    ...
    -------
        streamers: geopandas.GeoDataFrame
            GeoDataFrame containing different characteristics of the streamers
                ...
                "mean_var": mean of the variable used for the streamer calculations over all overturning grid cells
                ...
                "geometry": MultiPoint object with the coordinate points of the streamers in the format (x,y)
    """
    ...
    for (index,row) in tqdm(contours.iterrows(), total = contours.shape[0], leave = True, position = 0):
        ...
        if df_bp.empty:
            ...
            streamer_grids = [pd.DataFrame(bp_to_grid(row), columns = ["y", "x"]).astype("int") for index,row in df_bp.iterrows()]
            streamers.append(properties_per_timestep(row.date, streamer_grids, data, intensity, periodic_add, *args, **kwargs))
