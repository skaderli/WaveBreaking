...
def calculate_cutoffs(
    ...
    for name, group in tqdm(contours.groupby("date"), leave=True, position=0):
        ...
        def check_duplicates(df):
            ...
            temp = [
                [(p.y, p.x % kwargs["nlon"]) for p in row.geometry.geoms]
            ...
            return df[~df.reset_index(drop=True).index.isin(drop)].reset_index(
                drop=True
            )
                ...
                dist.pairwise(
                    (
                        np.radians(
                            np.asarray(
                                [
                                    (row.geometry.geoms[0].y, row.geometry.geoms[0].x),
                                    (
                                        row.geometry.geoms[-1].y,
                                        row.geometry.geoms[-1].x,
                                    ),
                                ]
                            )
                        )
        ...
        if df_co.empty:
            ...
            cutoff_grids = [
                pd.DataFrame(cutoff_to_grid(row), columns=["x", "y"]).astype("int")
                for index, row in df_co.iterrows()
            ]
            cutoffs.append(
